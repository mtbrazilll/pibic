...
public:
    ...
    QuadrantGuloso(const std::vector<int> &pts_idxs, double Xmax, double Ymax, double Xmin, double Ymin)
        : point_indices(pts_idxs), x_min(Xmin), x_max(Xmax), y_min(Ymin), y_max(Ymax){
...
struct SimpleQuadrant {
    ...
    SimpleQuadrant(const std::vector<int>& indices, double Xmax, double Ymax, double Xmin, double Ymin)
        : point_indices(indices), x_min(Xmin), x_max(Xmax), y_min(Ymin), y_max(Ymax)
    {}
...
class SpatialGrid {
public:
    SpatialGrid(double x_min, double x_max, double y_min, double y_max, double cell_size)
    ...
    void query(const Point& center, double radius, std::vector<int>& result) const {
        double cx = center.x();
        double cy = center.y();
...
// Helper function for optimized CGAL solution using Spatial Grid
int solve_cgal_grid(bool use_randomness, double x_max, double y_max, double x_min, double y_min) {
    ...
    pilha.push_back(SimpleQuadrant(indices, x_max, y_max, x_min, y_min));
    ...
    while (!pilha.empty()) {
        ...
        for (int idx : current.point_indices)
        {
            ...
            if (px >= rand1 && px <= current.x_max && py >= rand2 && py <= current.y_max)
                q1.addPoint(idx, p.point);
            else if (px >= current.x_min && px <= rand1 && py >= rand2 && py <= current.y_max)
                q2.addPoint(idx, p.point);
            else if (px >= current.x_min && px <= rand1 && py >= current.y_min && py <= rand2)
        ...
        if (!q1.point_indices.empty()) processQuadrant(q1);
        if (!q2.point_indices.empty()) processQuadrant(q2);
        if (!q3.point_indices.empty()) processQuadrant(q3);
        if (!q4.point_indices.empty()) processQuadrant(q4);
    }
...
int solve_cgal_impl(bool use_randomness, double x_max, double y_max, double x_min, double y_min) {
    ...
    while (!fila.empty()) {
        ...
        // Note: The actual bounds of sub-quadrants will be tight bounds of points inside them.
        // But we use these split lines to decide where points go.
        
        // Cria quadrantes vazios
        SimpleQuadrant q1, q2, q3, q4;
        // Reserve memory to avoid reallocations? We don't know size, but average is N/4.
        size_t reserve_size = current.point_indices.size() / 4 + 1;
        q1.point_indices.reserve(reserve_size);
        q2.point_indices.reserve(reserve_size);
        q3.point_indices.reserve(reserve_size);
        ...
        for (int idx : current.point_indices)
        {
            ...
            else // if (px >= rand1 && px <= current.x_max && py >= current.y_min && py <= rand2)
            {
                q4.addPoint(idx, p.point);
            }
        }
        ...
        // Lambda para processar um quadrante e decidir se subdivide ou gera componente
        // Reutiliza o vetor de vizinhos para evitar realocações
        std::vector<int> neighbor_indices;
...
int generate_solution_guloso(const std::vector<Ponto>& points,
                               ...
                               double x_min, double y_min, double p)
    {
        ...
        std::vector<int> initial_indices(points.size());
        for(size_t i=0; i<points.size(); ++i) initial_indices[i] = points[i].indice;
        ...
        while (!fila.empty()) {
            QuadrantGuloso current = fila.front();
            fila.pop();
            ...
            auto processQuadrant = [&](QuadrantGuloso& q, double mx, double my, double mnx, double mny) {
                if (q.point_indices.size() == 1) {
                    // Já temos um ponto único
                    Point centro = q.getCenter();
                    Fuzzy_sphere_Index sphere(centro, raio);
...
std::vector<Point> init_centers_enhanced(const std::vector<int> &point_indices)
{
    std::vector<Point> centers;
    ...
    std::vector<double> min_dist_sq(point_indices.size(), std::numeric_limits<double>::max());

    // Inicializa remaining_indices e min_dist_sq
    for (size_t i = 0; i < point_indices.size(); ++i)
    {
        if (pontos[point_indices[i]].point != centers[0]) // Evita adicionar o primeiro centro novamente
        {
            remaining_indices.push_back(point_indices[i]);
            min_dist_sq[i] = CGAL::squared_distance(pontos[point_indices[i]].point, centers[0]);
        }
    }
    ...
    while (!remaining_indices.empty())
    {
        ...
        for (size_t i = 0; i < remaining_indices.size(); ++i)
        {
            ...
            for (const Point &c : centers)
            {
                ...
                current_min_squared_dist = std::min(current_min_squared_dist, squared_dist);
            }
        ...
        if (max_idx_in_remaining != -1)
        {
            ...
            std::swap(remaining_indices[max_idx_in_remaining], remaining_indices.back());
...
std::vector<std::vector<int>> assign_to_clusters_enhanced(
    const std::vector<int> &demand_indices,
...
#include <chrono>
    ...
    double tolerance = 0.001;
    double raio2_tolerance = raio * raio + tolerance; // Para verificar cobertura com tolerância
    ...
    while (true)
    {
        ...
        std::vector<std::vector<int>> clusters = assign_to_clusters_enhanced(point_indices, centers);
        ...
        k = centers.size(); // Atualiza k com o número real de centros após min_circle
        ...
        else
        {
            ...
            if (all_covered)
            {
                ...
                if (k < best_k)
                {
                    ...
                    if (k > 1) { // Only remove if there's more than one center
                        centers.pop_back();
                        k--;
                    } else { // If only one center and it's the best, we can't reduce further
                        break;
                    }
                }
                else
            ...
            else
            {
                ...
                if (k >= best_k && best_k < (int)point_indices.size()) {
                     ...
                     while(centers.size() >= best_k) centers.pop_back();
                     k = centers.size();
                }
            }
        }
        ...
        if (!centers_changed && !all_covered && k >= best_k) { // If converged, not all covered, and k is not improving, break
            break;
        }
        if (k == 0 && !point_indices.empty()) { // Avoid infinite loop if k becomes 0 and points are not covered
            break;
        }
    }
...
int generate_solution_dr_enhanced_leve(const std::vector<Ponto> &points)
{
    ...
    for(size_t i=0; i<points.size(); ++i) point_indices[i] = points[i].indice;
            ...
            if (first_no_change)
            {
                ...
                if (!cluster_radii.empty() && !clusters.empty())
                {
                    ...
                    if (largest_idx < clusters.size() && !clusters[largest_idx].empty())
                    {
                        std::uniform_int_distribution<> distr(0, clusters[largest_idx].size() - 1);
                        int random_idx = distr(gen);
                        centers[largest_idx] = pontos[clusters[largest_idx][random_idx]].point;
                        centers_changed = true;
                    }
                }
            }
...
void distributePointsSimple(const std::vector<int> &pts_idxs,
                      SimpleQuadrant &q1, double XminQ1, double XmaxQ1, double YminQ1, double YmaxQ1,
                      SimpleQuadrant &q2, double XminQ2, double XmaxQ2, double YminQ2, double YmaxQ2,
                      SimpleQuadrant &q3, double XminQ3, double XmaxQ3, double YminQ3, double YmaxQ3,
                      SimpleQuadrant &q4, double XminQ4, double XmaxQ4, double YminQ4, double YmaxQ4)
{
                       ...
                       double x_min, double y_min)
{
    ...
    while (!fila.empty())
    {
        ...
        // Distribuir pontos
        distributePointsSimple(current.point_indices,
        ...
        // Lambda para processar um quadrante e decidir se subdivide ou gera componente
        auto processQuadrant = [&](SimpleQuadrant &q, double mx, double my, double mnx, double mny)
        {
            if (q.point_indices.size() == 1)
            {
                // Já temos um ponto único
                ...
                for(const auto& p : neighbors) neighbor_indices.push_back(p.indice);
                       ...
                       int max_depth)  // Parâmetro para profundidade máxima
{
    ...
    struct QuadrantWithDepth {
        ...
        QuadrantWithDepth(const std::vector<int>& pts_idxs, double x_max, double y_max, 
    ...
    while (!fila.empty())
    {
        ...
        if (current.depth >= max_depth)
        {
            ...
            solution = generate_solution_dr_enhanced(current.quad.point_indices) + solution;
        ...
        // Distribuir pontos
        distributePointsSimple(current.quad.point_indices,
                         q1, XminQ1, XmaxQ1, YminQ1, YmaxQ1,
                         q2, XminQ2, XmaxQ2, YminQ2, YmaxQ2,
                         q3, XminQ3, XmaxQ3, YminQ3, YmaxQ3,
                         q4, XminQ4, XmaxQ4, YminQ4, YmaxQ4);
    ...
    for(int idx : ranking) {
        ...
        for (Bola* bola_ptr : neighbors) {
            ...
            if (bola.mc == nullptr) {
                 ...
                 for(int p_idx : bola.pontos_indices) pts.push_back(pontos[p_idx].point);
            ...
            // Strategy: Try to add point, build new MC, verify ALL points. If fail, revert.
            
            // Optimization: First check if incremental MC (from support points) is promising
            Min_circle mc_test;
            for (auto it = bola.mc->support_points_begin(); it != bola.mc->support_points_end(); ++it) {
                mc_test.insert(*it);
            }
            ...
            if (mc_test.circle().squared_radius() <= raio * raio - epsilon) {
                 ...
                 for(int p_idx : bola.pontos_indices) current_pts.push_back(pontos[p_idx].point);
                 
                 Min_circle* new_mc = new Min_circle(current_pts.begin(), current_pts.end(), false);
                 ...
                 if (new_mc->circle().squared_radius() > raio * raio + epsilon) {
                     ...
                     for(const auto& p_idx : bola.pontos_indices) {
                         double d = CGAL::squared_distance(new_mc->circle().center(), pontos[p_idx].point);
                 ...
                 if (valid) {
                     // Commit
                     std::pair<int, int> old_cell = get_grid_idx(bola.centro); // Capture OLD center
