...
void CMSA(float time_limit, int max_age) {
  ...
  // ================= CMSA Loop ==========================
  std::vector<bool> tabu_list(pontos.size(), false);
  while (total_duration < computation_time_limit) {
    ...
    for (int na = 0; na < n_of_sols; na++) {
      ...
      if (algo == 0) {
        ...
        generate_divise_dr_profundidade(pontos, max_x, max_y, min_x, min_y,
                                        profundidade);
      } else if (algo == 3) {
        cmsa_dr_particao(tabu_list);
      } else if (algo == 4) {
        fastCover();
      } else if (algo == 5) {
        if (loops % 2 == 1) {
    ...
    if (loops_with_no_improval > 10) {
      ...
      for (const auto &c : manager.components) {
        double prob = (double)c.vida / max_vida;
